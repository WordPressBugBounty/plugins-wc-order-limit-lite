...
/**
 *
 * Single product rule page of plugin.
 *
...
$xs_i                      = 0;
?>
<div id="wcol_tab" class="panel woocommerce_options_panel wc-metaboxes-wrapper xswcol-rules">

	<div class="toolbar">
		<div class="">
			<span class="spinner wcol_spinner"></span>
			<a id="wcol_add_new_rule" data-id="product" class="button" href="#"><?php esc_html_e( 'Add New Rule', 'order-limit-for-woocommerce' ); ?></a>
	...
	<div class="wc-metaboxes">
		...
		if ( is_array( $wcol_rules ) ) {
			foreach ( $wcol_rules as $mkey => $wcol_options ) {
				$max_limit = $wcol_options['max-rule-limit'];
				$min_limit = $wcol_options['min-rule-limit'];
				...
				?>
				<div class="wc-metabox closed <?php echo ( isset( $wcol_options['accomulative'] ) && 'on' === $wcol_options['accomulative'] ) ? 'wcol_accomulative_rule' : ''; ?>">
					<h3>
						<?php
						$allowed_html = array(
							'span' => array(),
					...
					<div class="wc-metabox-content" style="display:none;">
						...
						<div class="options_group">
							<p class="form-field">
								...
								<select class="across-all-orders-limit <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[across-all-orders][<?php echo esc_html( $mkey ); ?>]">
									...
									<option value="2" <?php echo ( isset( $wcol_options['across-all-orders'] ) && '2' === $wcol_options['across-all-orders'] ) ? 'selected="selected"' : ''; ?>>
										<?php esc_html_e( 'Previous Order of Current User', 'order-limit-for-woocommerce' ); ?>
									</option>
									<option value="3" <?php echo ( isset( $wcol_options['across-all-orders'] ) && '3' === $wcol_options['across-all-orders'] ) ? 'selected="selected"' : ''; ?>>
										<?php esc_html_e( 'Previous Order of All Users', 'order-limit-for-woocommerce' ); ?>
									</option>
								</select>
							</p>
						...
						<div class="options_group">
							...
							<p class="form-field <?php echo ( 'on' !== $wcol_options['enable-time-limit'] ) ? 'wcol-hidden' : ''; ?>">
								...
								<select class="wcol-rule-time-span <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[rule-time-span][<?php echo esc_html( $mkey ); ?>]">
									...
									<option value="weekly"  <?php echo ( isset( $wcol_options['rule-time-span'] ) && 'weekly' === $wcol_options['rule-time-span'] ) ? 'selected' : ''; ?>>
										<?php esc_html_e( 'Weekly', 'order-limit-for-woocommerce' ); ?>
									</option>
									<option value="monthly"  <?php echo ( isset( $wcol_options['rule-time-span'] ) && 'monthly' === $wcol_options['rule-time-span'] ) ? 'selected' : ''; ?>>
										<?php esc_html_e( 'Monthly', 'order-limit-for-woocommerce' ); ?>
									</option>
									<option value="yearly"  <?php echo ( isset( $wcol_options['rule-time-span'] ) && 'yearly' === $wcol_options['rule-time-span'] ) ? 'selected' : ''; ?>>
										<?php esc_html_e( 'Yearly', 'order-limit-for-woocommerce' ); ?>
									</option>
									<option value="days"  <?php echo ( isset( $wcol_options['rule-time-span'] ) && 'days' === $wcol_options['rule-time-span'] ) ? 'selected' : ''; ?>>
										<?php esc_html_e( 'Custom days', 'order-limit-for-woocommerce' ); ?>
									</option>
									<option value="custom"  <?php echo ( isset( $wcol_options['rule-time-span'] ) && 'custom' === $wcol_options['rule-time-span'] ) ? 'selected' : ''; ?>>
										<?php esc_html_e( 'Custom dates', 'order-limit-for-woocommerce' ); ?>
									</option>
								</select>
							</p>
							<p class="form-field <?php echo ( 'yearly' !== $wcol_options['rule-time-span'] ) ? 'wcol-hidden' : ''; ?>" >
								<label><?php esc_html_e( 'Year Start Date:', 'order-limit-for-woocommerce' ); ?></label>
								<input type="number" max="31" min="01" class="wcol-yearly-start-day <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[yearly-start-day][<?php echo esc_html( $mkey ); ?>]" value="<?php echo esc_html( $wcol_options['yearly-start-day'] ); ?>"/>
								<select class="wcol-yearly-start-month <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[yearly-start-month][<?php echo esc_html( $mkey ); ?>]">
									<?php
							...
							<p class="form-field <?php echo ( 'weekly' !== $wcol_options['rule-time-span'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'Week Start Day:', 'order-limit-for-woocommerce' ); ?></label>
								<select class="wcol-weekly-start-day <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[weekly-start-day][<?php echo esc_html( $mkey ); ?>]">
									<?php
							...
							<p class="form-field <?php echo ( 'monthly' !== $wcol_options['rule-time-span'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'Month Start Date:', 'order-limit-for-woocommerce' ); ?></label>
								<input class="wcol-monthly-start-date <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" type="number" min="01" max="31" name="wcol_rules[monthly-start-date][<?php echo esc_html( $mkey ); ?>]" value="<?php echo esc_html( $wcol_options['monthly-start-date'] ); ?>"/>
							</p>
							<p class="form-field <?php echo ( 'days' !== $wcol_options['rule-time-span'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'Number of days:', 'order-limit-for-woocommerce' ); ?></label>
								<input class="wcol-rule-days <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" type="number" min="0" name="wcol_rules[days][<?php echo esc_html( $mkey ); ?>]" value="<?php echo esc_html( $wcol_options['days'] ); ?>"/>
							</p>
							<p class="form-field <?php echo ( 'custom' !== $wcol_options['rule-time-span'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'Start Time:', 'order-limit-for-woocommerce' ); ?></label>
								<input type="text" class="wcol-rule-start-time <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[rule-start-time][<?php echo esc_html( $mkey ); ?>]" value="<?php echo isset( $wcol_options['rule-start-time'] ) ? esc_html( $wcol_options['rule-start-time'] ) : ''; ?>" />
							</p>
							<p class="form-field <?php echo ( 'custom' !== $wcol_options['rule-time-span'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'End Time:', 'order-limit-for-woocommerce' ); ?></label>
								<input type="text" class="wcol-rule-end-time <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[rule-end-time][<?php echo esc_html( $mkey ); ?>]" value="<?php echo isset( $wcol_options['rule-end-time'] ) ? esc_html( $wcol_options['rule-end-time'] ) : ''; ?>" />
							</p>
						</div>
						...
						<div class="options_group">
							...
							<p class="form-field <?php echo ( 'on' !== $wcol_options['enable-for-users'] ) ? 'wcol-hidden' : ''; ?>">
								...
								<select class="wcol-rule-user-type <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[user-type][<?php echo esc_html( $mkey ); ?>]" >
									<option value="guest-users" <?php echo ( 'guest-users' === $wcol_options['user-type'] ) ? 'selected' : ''; ?>>
										<?php esc_html_e( 'Guest Users Only', 'order-limit-for-woocommerce' ); ?>
									</option>
									<option value="specific-users" <?php echo ( 'specific-users' === $wcol_options['user-type'] ) ? 'selected' : ''; ?>>
										<?php esc_html_e( 'Specific Users Only', 'order-limit-for-woocommerce' ); ?>
									</option>
									<option value="specific-roles" <?php echo ( 'specific-roles' === $wcol_options['user-type'] ) ? 'selected' : ''; ?>>
										<?php esc_html_e( 'Specific Roles Only', 'order-limit-for-woocommerce' ); ?>
									</option>
									<option value="all-users" <?php echo ( 'all-users' === $wcol_options['user-type'] ) ? 'selected' : ''; ?>>
							...
							<p class="form-field  <?php echo ( 'on' !== $wcol_options['enable-for-users'] || 'guest-users' !== $wcol_options['user-type'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'Guest Users:', 'order-limit-for-woocommerce' ); ?></label>
								<select class="wcol-select-guest-users wcol-rule-guest-users <?php echo ( isset( $wcol_options['disable-limit'] ) && 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol-rules[rule-guest-users][<?php echo esc_html( $mkey ); ?>][]" multiple="multiple" >
							...
							<p class="form-field <?php echo ( 'on' !== $wcol_options['enable-for-users'] || 'specific-users' !== $wcol_options['user-type'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'Users:', 'order-limit-for-woocommerce' ); ?></label>
								<?php
								$user_ids = array();
								...
								?>
								<select class="wcol-select-users wcol-rule-users <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[rule-users][<?php echo esc_html( $mkey ); ?>][]" multiple="multiple" >
									<?php
							...
							<p class="form-field <?php echo ( 'on' !== $wcol_options['enable-for-users'] || 'specific-roles' !== $wcol_options['user-type'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'Roles:', 'order-limit-for-woocommerce' ); ?></label>
								<?php
								$user_roles = array();
								...
								?>
								<select class="wcol-select-roles wcol-rule-roles <?php echo ( 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol_rules[rule-roles][<?php echo esc_html( $mkey ); ?>][]" multiple="multiple" >
									<?php
						...
						<div class="options_group">
							...
							<p class="form-field wcol-payment-method <?php echo ( ! isset( $wcol_options['enable-for-payment'] ) || 'on' !== $wcol_options['enable-for-payment'] ) ? 'wcol-hidden' : ''; ?>">
								<label><?php esc_html_e( 'Payment Method', 'order-limit-for-woocommerce' ); ?>:</label>
								<select class="wcol-rule-payment-type <?php echo ( isset( $wcol_options['disable-limit'] ) && 'on' === $wcol_options['disable-limit'] ) ? 'wcol-disabled' : ''; ?>" name="wcol-rules[payment-method][<?php echo esc_html( $mkey ); ?>][]" multiple="multiple">
									<?php
		...
		?>
		<input type="hidden" class="xswcol-spid" class="xswcol-spid" value="<?php echo esc_html( $xs_i ); ?>">
	</div>
</div>
