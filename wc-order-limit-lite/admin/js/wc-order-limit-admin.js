jQuery(document).ready(function ($) {
    ...
    $('#wcol_add_new_rule').on('click', function (e) {
        ...
        if (window.location.href.indexOf('post.php') > 0 || window.location.href.indexOf('post-new') > 0) {
            ...
            $.ajax({
                ...
                type: 'post',
                data: { 'action': 'wcol_load_new_row', 'wcol_spid': wcol_rid, 'rule_type': rule_type },
            ...
            $.ajax({
                url: wcol_script_vars.ajax_url,
                type: 'post',
                data: { 'action': 'wcol_load_new_row', 'wcol_spcid': wcol_rid, 'rule_type': rule_type },
                success: function (res) {
    ...
    jQuery('#wcol-add-exclude-rule').on('click', function () {
        ...
        jQuery.ajax({
            url: wcol_script_vars.ajax_url,
            type: 'post',
            data: { 'action': 'wcol_load_new_row', 'rule_type': 'exclude', 'wcol_eid': wcol_rid },
            success: function (res) {
                wcol_rid++;
